...
const ChatContainer = () => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-gradient-to-br from-slate-50 via-blue-50 to-sky-100">
      <ChatHeader />

      <div className="flex-1 overflow-y-auto p-3 sm:p-4 lg:p-6 space-y-3 sm:space-y-4 bg-gradient-to-br from-sky-50/30 via-blue-50/40 to-indigo-50/30">
        {!messages || messages?.length === 0 ? (
          ...
          messages?.map((message, index) => {
            ...
            return (
              <div
                key={message._id}
                className={`flex w-full group/msg ${isOwnMessage ? "justify-end" : "justify-start"} ${!showAvatar ? 'mt-1 sm:mt-2' : 'mt-3 sm:mt-4'}`}
                    ...
                    <div className="flex items-center gap-1.5 group/bubble relative">
                      {isOwnMessage && (
                        <button
                          ...
                          className="opacity-0 group-hover/msg:opacity-100 transition-opacity p-1 text-gray-400 hover:text-red-500 rounded-full hover:bg-gray-200/50 flex-shrink-0"
                      ...
                      <div className={`relative px-3 sm:px-4 py-2 sm:py-3 rounded-2xl border shadow-sm min-w-0 max-w-full ${
                        isOwnMessage 
                          ? 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white border-blue-400/30 rounded-br-md shadow-blue-200/30' 
